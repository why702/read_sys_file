...
class LOG():
    def __init__(self):
        self.keywords = [
            "et", "hc", "egp", "p", "ip", "rl", "irl", "rls", "sl", "mica", "dry", "sP", "isp", "q3", "ispd", "q3d", "qty"
        ]
        self.dict = {}
        ...
        for root, dirs, files in os.walk(dir_path, topdown=False):
            for name in files:
                ...
                if os.path.splitext(name)[1] != '.png':
                    continue
                elif name.find('_Img8b_') != -1:
                    continue
                elif name.find('_Img16bBkg_') != -1:
                    continue

                log = parse_file_name(root, name)
